...
from .models import Job, JobSkill, Candidate
...
class Database:
    ...
    def UPDATE_user_profile(
        cls, user_id: int, username: str, email: str, company_name: str
    ) -> tuple[bool, str]:
        """Update username, email, and company_name for a user."""
        try:
            affected = cls.execute_set(
                f"""
                UPDATE {UsersCol.TABLE_NAME}
                SET {UsersCol.NAME} = ?, {UsersCol.EMAIL} = ?, {UsersCol.COMPANY_NAME} = ?
                WHERE {UsersCol.ID} = ?
                """,
                (username, email, company_name, user_id),
            )
            if not affected:
            ...
            error_msg = str(e)
            if UsersCol.NAME in error_msg:
                return False, f"Username '{username}' is already taken."
            elif UsersCol.EMAIL in error_msg:
    ...
    def UPDATE_user_password(
        cls, user_id: int, new_password_hash: str
    ) -> tuple[bool, str]:
    ...
    def DELETE_user(cls, user_id: int) -> tuple[bool, str]:
        ...
        try:
            affected = cls.execute_set(
                f"""
                DELETE FROM {UsersCol.TABLE_NAME}
                WHERE {UsersCol.ID} = ?
                """,
                (user_id,),
            )
        ...
        for job in jobs_data:
            ...
            result.append(
                Job(
                    ...
                    min_edu_weight=job[JobsCol.MIN_EDU_WEIGHT],
                    min_exp_weight=job[JobsCol.MIN_EXP_WEIGHT],
                )
            )
    ...
    def SELECT_job(cls, user_id: int, job_id: int) -> Job | None:
        ...
        return Job(
            ...
            skillname_skillweight_dict=skill_name_weight,
            min_edu_weight=job_data[JobsCol.MIN_EDU_WEIGHT],
            min_exp_weight=job_data[JobsCol.MIN_EXP_WEIGHT],
        )
    ...
    def DELETE_job(cls, user_id: int, job_id: int) -> tuple[bool, str]:
        ...
        try:
            ...
            if affected_rows and affected_rows > 0:
                ...
                return False, "Error deleting the job!"
    ...
    def INSERT_candidate(
        ...
        """Add A New Candidate To The candidates Table And Returns True If Added Successfully."""
        ...
        try:
            ...
            error_msg = str(e)
            if (
                CandidatesCol.EMAIL in error_msg
                or f"{CandidatesCol.TABLE_NAME}.{CandidatesCol.EMAIL}" in error_msg
            ):
                return (
                    False,
                ...
                or f"{CandidatesCol.TABLE_NAME}.{CandidatesCol.PHONE}" in error_msg
            ):
                return (
                    False,
    ...
    @classmethod
    def SELECT_candidates(cls, user_id: int, job_id: int) -> list[Candidate]:
        """Get All Candidates For A Specific Job And User"""

        candidates_dict = cls.execute_select(
            f"""
            SELECT * FROM {CandidatesCol.TABLE_NAME}
            WHERE {CandidatesCol.USER_ID} = ? AND {CandidatesCol.JOB_ID} = ?
            ORDER BY {CandidatesCol.MATCH_SCORE} DESC, {CandidatesCol.CREATED_AT} DESC
            """,
            (user_id, job_id),
        )
        ...
        for c in candidates_dict:
            result.append(
                Candidate(
                    id=c[CandidatesCol.ID],
                    job_id=c[CandidatesCol.JOB_ID],
                    user_id=c[CandidatesCol.USER_ID],
                    name=c[CandidatesCol.NAME],
                    email=c[CandidatesCol.EMAIL],
                    phone=c[CandidatesCol.PHONE],
                    filename=c[CandidatesCol.FILENAME],
                    ...
                    match_score=c[CandidatesCol.MATCH_SCORE],
                    created_at=c[CandidatesCol.CREATED_AT],
                )
            )
    ...
    @classmethod
    def SELECT_candidate(cls, candidate_id: int, user_id: int) -> Candidate | None:
        """Get A Specific Candidate By ID For A Specific User"""

        candidate_data = cls.execute_select_one(
            f"""
            SELECT * FROM {CandidatesCol.TABLE_NAME} WHERE {CandidatesCol.ID} = ? AND {CandidatesCol.USER_ID} = ?
            """,
        ...
        return Candidate(
            id=candidate_data[CandidatesCol.ID],
            job_id=candidate_data[CandidatesCol.JOB_ID],
            user_id=candidate_data[CandidatesCol.USER_ID],
            name=candidate_data[CandidatesCol.NAME],
            email=candidate_data[CandidatesCol.EMAIL],
            phone=candidate_data[CandidatesCol.PHONE],
            filename=candidate_data[CandidatesCol.FILENAME],
            ...
            match_score=candidate_data[CandidatesCol.MATCH_SCORE],
            created_at=candidate_data[CandidatesCol.CREATED_AT],
        )

    @classmethod
    def SELECT_candidates_as_dicts(
        cls, user_id: int, job_id: int
    ) -> list[dict[str, Any]]:
        """Get All Candidates For A Specific Job And User"""
    ...
    @classmethod
    def DELETE_candidate(cls, candidate_id: int, user_id: int) -> tuple[bool, str]:
        """Delete a candidate by ID and user ID to ensure they can only delete their own candidates"""
        try:
            affected_rows = cls.execute_set(
                f"DELETE FROM {CandidatesCol.TABLE_NAME} WHERE {CandidatesCol.ID} = ? AND {CandidatesCol.USER_ID} = ?",
                (candidate_id, user_id),
            )
            ...
            if affected_rows and affected_rows > 0:
                ...
                return False, "Error deleting the candidate!"
...
"""
sqlite3 instance/skillmatch.db
.mode box
SELECT * FROM users;
"""
